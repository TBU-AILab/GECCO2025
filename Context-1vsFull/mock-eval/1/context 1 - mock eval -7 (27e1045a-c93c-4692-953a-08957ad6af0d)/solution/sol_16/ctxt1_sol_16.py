#To resolve the persistent `KeyError('run')` and ensure everything is functioning correctly, I'll make sure the test setup and usage for the `run` method are straightforward and integrated properly. Here is an updated, refined version of the PSO with a clear focus on ensuring the `run` method is ready for direct invocation:
#
import numpy as np
